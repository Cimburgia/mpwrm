...
float *get_power_arr(CFDictionaryRef pwr_sample){
    float *pwr_arr = malloc(num_cores * sizeof(float));
    __block int i = 0;
    // Get number of indicies 8 or 18 depending on E vs. P
    IOReportIterate(pwr_sample, ^int(IOReportSampleRef sample) {
        CFStringRef chann_name  = IOReportChannelGetChannelName(sample);
...
int main(int argc, char* argv[]) {
    // keep samples
    float **power_samples = malloc(10*sizeof(float*));
    float **power_deltas = malloc(10*sizeof(float*));

    // initialize cmd and cpu data
    init_unit_data();
    CFDictionaryRef *samples = malloc(10*sizeof(CFDictionaryRef));

    // take samples and save
    for (int i = 0; i < 10; i++){    
        samples[i] = IOReportCreateSamples(unit->pwr_sub, unit->pwr_sub_chann, NULL);
        sleep(5);
    }
    // Extract raw values and compute delta
    CFDictionaryRef prev = samples[0];
    for (int i = 0; i < 10; i++){
        power_samples[i] = get_power_arr(samples[i]);
        CFDictionaryRef pwr_delta = IOReportCreateSamplesDelta(prev, samples[i], NULL);
        power_deltas[i] = get_power_arr(pwr_delta);
        prev = samples[i];
    }
    ...
    char sample_filename[150];
    char delta_filename[150];
    sprintf(sample_filename, "data/power_samples_%s.txt", time_str);
    sprintf(delta_filename, "data/power_deltas_%s.txt", time_str);

    // Write samples
    FILE *file = fopen(sample_filename, "w");
    for (int i = 0; i < num_cores; i++) {
        for (int j = 0; j < 10; j++) {
            fprintf(file, "%.1f\t", power_samples[j][i]);
        }
        fprintf(file, "\n");
    }
    ...
    // Write deltas
    FILE *filed = fopen(delta_filename, "w");
    for (int i = 0; i < num_cores; i++) {
        for (int j = 0; j < 10; j++) {
            fprintf(filed, "%.1f\t", power_deltas[j][i]);
        }
        fprintf(filed, "\n");
    }
